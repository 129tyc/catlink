...
API_C07_EVENT_TIMELINE = "token/litterbox/stats/log/timeline"
...
class C07Device(LitterDevice):
    ...
    async def async_init(self) -> None:
        ...
        if self.event_coordinator is None:
            self.event_coordinator = DataUpdateCoordinator(
                self.account.hass,
                _LOGGER,
    ...
    @property
    def state(self) -> str:
        """Return the broad device state using the existing device pattern."""
        return super().state
    ...
    def hass_sensor(self) -> dict:
        ...
        return {
            ...
            "last_event": {
                "icon": "mdi:history",
                "coordinator_attr": "event_coordinator",
                "state_attrs": self.last_event_attrs,
            },
            "error": {
    ...
    @property
    def hass_image(self) -> dict:
        """Return fixed image entities for the latest C07 event."""
        return {
            "last_event_before": {
            ...
            "last_event_after": {
                "name": "Last event after",
                "coordinator_attr": "event_coordinator",
            },
            "last_event_on_site": {
                "name": "Last event on site",
                "coordinator_attr": "event_coordinator",
            },
        }
    ...
    def event_image_url(self, role: str) -> str | None:
        ...
        record = self.event_data.get("last_event") or {}
        role_map = {
            "last_event_before": "before_image",
            "last_event_after": "after_image",
            "last_event_on_site": "on_site_image",
            "last_event_thumbnail": "thumbnail",
        }
        return record.get(role_map.get(role, ""))
    ...
    async def update_events(self) -> dict:
        """Refresh today's C07 events without affecting device availability."""
        previous = self.event_data
        try:
            timezone = dt_util.get_time_zone(self.account.hass.config.time_zone)
            date = dt_util.now(timezone).date().isoformat()
            response = await self.account.request(
                API_C07_EVENT_TIMELINE,
                {
                    "deviceId": self.id,
                    "date": date,
                    "type": 0,
            ...
            if not response:
                raise ValueError("C07 event request returned no response")
            if response.get("returnCode") not in (None, 0, "0"):
                raise ValueError(f"C07 event request code {response.get('returnCode')}")
            data = response.get("data", {})
            if not isinstance(data, dict):
                raise ValueError("C07 event response data is not an object")
            raw_records = data.get("records")
            if not isinstance(raw_records, list):
                raise ValueError("C07 event records are not a list")
            ...
            for record in records:
                ...
                await self._resolve_event_image(record)
                old = previous_by_id.get(str(record.get("event_id")))
                if old:
                    for key in (
                        "before_image",
                        "after_image",
            ...
            _LOGGER.warning("Got C07 events for %s failed: %s", self.name, exc)
        return self.event_data or previous

    async def _resolve_event_image(self, record: dict[str, Any]) -> None:
        """Resolve missing before/after image URLs from their picture IDs."""
        for url_key, id_key in (
            ("before_image", "before_image_id"),
            ("after_image", "after_image_id"),
            ("on_site_image", "on_site_image_id"),
        ):
            if record.get(url_key) or not record.get(id_key):
                continue
            try:
                response = await self.account.request(
                    API_C07_GET_PIC_URL,
                    {"deviceId": self.id, "picId": record[id_key]},
                )
                data = response.get("data", {}) if response else {}
                if isinstance(data, dict):
                    image_url = data.get("picUrl") or data.get("url")
                    record[url_key] = image_url
                    _LOGGER.debug(
                        "C07 event image resolved role=%s id_present=%s returnCode=%s url_present=%s",
                        url_key,
                        bool(record.get(id_key)),
                        response.get("returnCode") if response else None,
                        bool(image_url),
                    )
            except (TypeError, ValueError) as exc:
                _LOGGER.debug(
                    "Resolve C07 event image failed role=%s error=%s",
                    url_key,
                    type(exc).__name__,
                )
    ...
    async def select_box_full_sensitivity(self, level, **kwargs) -> bool:
        ...
        if level_code is None:
            ...
            return False
        if self.box_full_sensitivity == level:
            return True
        response = await self.account.request(
    ...
    async def _set_toggle(
        ...
        if state_key is not None:
            current = self.detail.get(state_key)
            if current not in (None, "") and self._flag(current) == enable:
                return True
        response = await self.account.request(
            api,
    ...
    async def async_set_quiet_mode(self, enable: bool, **kwargs) -> bool:
        ...
        current = self.detail.get("quietEnable")
        if current not in (None, "") and self._flag(current) == enable:
            return True
    ...
    async def async_set_voice_prompt(self, enable: bool, **kwargs) -> bool:
        ...
        return await self._set_toggle(
            API_C07_VOICE_PROMPT_SWITCH,
            enable,
            "C07 voice prompt",
            state_key="voiceEnable",
        )
    ...
    async def async_set_panel_tone(self, enable: bool, **kwargs) -> bool:
        ...
        return await self._set_toggle(
            API_C07_PANELTONE, enable, "C07 panel tone", state_key="paneltone"
        )
    ...
    async def select_pave_level(self, value, **kwargs) -> bool:
        ...
        if level is None:
            ...
            return False
        if self.pave_level_control == value:
            return True
        response = await self.account.request(
            API_C07_PAVE_LEVEL,
    ...
    async def select_litter_type(self, value, **kwargs) -> bool:
        ...
        if litter_type is None:
            ...
            return False
        if self.litter_type_control == value:
            return True
        response = await self.account.request(
    ...
    async def select_safe_time(self, value, **kwargs) -> bool:
        ...
        if safe_time is None:
            ...
            return False
        if self.safe_time_control == value:
            return True
        response = await self.account.request(
